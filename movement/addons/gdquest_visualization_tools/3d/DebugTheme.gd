...
func _init(node: Node3D):
	_node = node
	is_implemented = _node is CollisionPolygon3D or _node is RayCast3D or _node is Path3D
...
func free_rids() -> void:
	for key in rids:
		for rid in rids[key]:
			funcref(RenderingServer, "free_rid").call_func(rid)
		rids[key].clear()
...
func draw_meshes(meshes_info: Array) -> void:
	...
	for mesh_info in meshes_info:
		...
		RenderingServer.mesh_add_surface_from_arrays(mesh_RID, mesh_info.primitive_type, mesh_info.arrays)
		RenderingServer.mesh_surface_set_material(mesh_RID, 0, material_RID)
		rids.instances.push_back(RenderingServer.instance_create2(mesh_RID, _node.get_world_3d().scenario))
...
func set_visible(new_visible: bool) -> void:
	...
	for rid in rids.instances:
		RenderingServer.instance_set_visible(rid, new_visible)


func _set_disabled_effect() -> void:
	if palette != DebugPalette.Type.DISABLED:
		_previous_palette = palette
	material.set_shader_parameter("color", DebugPalette.COLORS[palette])
...
func _set_theme_fresnel_power(new_theme_fresnel_power: float) -> bool:
	...
	material.set_shader_parameter("fresnel_power", theme_fresnel_power)
	return true


func _set_theme_edge_intensity(new_theme_edge_intensity: float) -> bool:
	theme_edge_intensity = new_theme_edge_intensity
	material.set_shader_parameter("edge_intensity", theme_edge_intensity)
	return true
